...
const Actions = ({ post: post_ }) => {
  ...
  const [liked, setLiked] = useState(post_.likes_count);
  ...
  const handleLikeAndUnlike = async () => {
    ...
    try {
      const res = await fetch("http://localhost:1000/like/p/" + post._id, {
        method: 'POST',
        headers: {
  ...
  const handleReply = async () => {
    if (!user) return showToast("Error", "You need to login to reply to a post", "error");
    if (isReplying) return
    setIsReplying(true);

    try {
      const res = await fetch("http://localhost:1000/comment/p/" + post._id, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <Flex flexDirection={"column"}>
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            <FormControl>
              <Input placeholder='Reply to post'
                value={reply}
